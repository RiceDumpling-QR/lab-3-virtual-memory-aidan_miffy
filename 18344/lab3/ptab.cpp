...
PTE pageTable::createEntry(unsigned long addr, size_t level){
  ...
  unsigned index = getEntryIdFromAddr(addr, level);
    PTE &entry = table[index];

    if (level == levels - 1) {
        if (entry.pte == nullptr) {
            entry.pte = (pageTableEntry *)calloc(1, sizeof(pageTableEntry));
        }
    } else {
        if (entry.pt == nullptr) {
            entry.pt = new pageTable();
        }
    }
    return entry;
}
...
PTE pageTable::getEntryDirect(unsigned long index){
  ...
  // useful when you alaready know the index 
...
unsigned pageTable::getEntryIdFromAddr(unsigned long addr, size_t level){
  ...
  */
  addr >>= VM_PPOBITS;
  size_t shift_amount = pageTableIndexBits * (levels - level - 1);
  unsigned long mask = (1UL << pageTableIndexBits) - 1UL;
  unsigned index = (addr >> shift_amount) & mask;
  assert(index < pageTableSize);
  return index;
